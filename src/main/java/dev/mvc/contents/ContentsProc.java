...
public class ContentsProc implements ContentsProcInter {
  ...
  @Override
  public ArrayList<ContentsVO> list_by_categono(int categono) {
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_categono(categono);
    return list;
  }
  ...
  @Override
  public ArrayList<ContentsVO> list_by_categono_search(HashMap<String, Object> hashMap) {
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_categono_search(hashMap);
    return list;
  }

  @Override
  public int list_by_categono_search_count(HashMap<String, Object> hashMap) {
    int cnt = this.contentsDAO.list_by_categono_search_count(hashMap);
    return cnt;
  }

  @Override
  public ArrayList<ContentsVO> list_by_categono_search_paging(HashMap<String, Object> map) {
    /*
    ...
    ArrayList<ContentsVO> list = this.contentsDAO.list_by_categono_search_paging(map);
   ...
   */
  @Override
  public String pagingBox(int categono, int now_page, String word, String list_file, int search_count, int record_per_page,
      int page_per_block) {
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (now_grp >= 2) { // 현재 그룹번호가 2이상이면 페이지수가 11페이지 이상임으로 이전 그룹으로 갈수 있는 링크 생성
      str.append("<span class='span_box_1'><A href='" + list_file + "?categono="+categono+"&word=" + word + "&now_page=" + _now_page
          + "'>이전</A></span>");
    }
    ...
    for (int i = start_page; i <= end_page; i++) {
      ...
      if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='" + list_file + "?categono="+categono+"&word=" + word + "&now_page=" + i + "'>" + i
            + "</A></span>");
      }
    }
  ...
  @Override
  public int delete_children(int categono) {
    int cnt = this.contentsDAO.delete_children(categono);
    return cnt;
  }

  @Override
  public int count_by_categono(int categono) {
    int cnt = this.contentsDAO.count_by_categono(categono);
    return cnt;
  }

  @Override
  public int delete_by_categono(int categono) {
    int cnt = this.contentsDAO.delete_by_categono(categono);
    return cnt;
  }

  @Override
  public int count_by_userno(int userno) {
    int cnt = this.contentsDAO.count_by_userno(userno);
    return cnt;
  }

  @Override
  public int delete_by_userno(int userno) {
    int cnt = this.contentsDAO.delete_by_userno(userno);
    return cnt;
  }
